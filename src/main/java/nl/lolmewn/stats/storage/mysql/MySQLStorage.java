...
public class MySQLStorage extends StorageManager {
    ...
    private Map<Stat, StatMySQLHandler> storers = new HashMap<>();
    ...
    private void registerStorers() {
        StatManager.getInstance().getStat("Blocks broken").ifPresent(stat -> this.storers.put(stat, new BlockBreakStorage()));
    }

    private void storeEntry(StatsPlayer player, StatsContainer container, StatTimeEntry entry) throws SQLException {
        if (this.storers.containsKey(container.getStat())) {
            try (Connection con = this.getConnection()) {
                this.storers.get(container.getStat()).storeEntry(con, player, container, entry);
            }
        }
    }
    ...
    public void internalLoadPlayer(StatsPlayer player) {
        try (Connection con = this.getConnection()) {
            for (Map.Entry<Stat, StatMySQLHandler> mapEntry : this.storers.entrySet()) {
